...
app.get('/api/goals', getUser, async (req: any, res) => {
  const { data, error } = await supabase
    .from('user_goals')
    .select('mandatory_goals, nice_to_have_goals, tentative_braindump, top_priority')
    .eq('user_id', req.user.id)
    .single()
  ...
  res.json(data || { mandatory_goals: '', nice_to_have_goals: '', tentative_braindump: '', top_priority: '' })
})

app.put('/api/goals', getUser, async (req: any, res) => {
  const { mandatory_goals, nice_to_have_goals, tentative_braindump, top_priority } = req.body
  ...
  if (top_priority !== undefined) updates.top_priority = top_priority
